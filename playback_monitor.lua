ardour {
	...
	description = [[
		...
		when recording so that it doesn't pollute monitoring.
	]]
}
...
function dsp_runmap (bufs, in_map, out_map, n_samples, offset)
	...
	local target_gain = Session:record_status() ~= ARDOUR.Session.RecordState.Disabled and 0.0 or 1.0; -- when recording, target_gain = 0.0; otherwise use 1.0
	-- apply I/O map
